...
import { createBlockWithTxDetails } from '../../../../test/rpc/create-blocks';
...
import { eth_getBlockTransactionCountByNumber } from './eth_getBlock';

describe('rpc method: eth_getBlockTransactionCountByNumber', () => {
    const reqId = 'someid';
    ...
    rpcUnitTest('returns 0 when no transactions match user addresses', async ({ reqContext, rpc, authorizer }) => {
        ...
        rpc.registerSend(`${reqId}_getBlockByNumber`, createBlockWithTxDetails());
        rpc.registerSend(`${reqId}_getBlockReceipts`, [
            createReceipt('0x4294364950d5b9c9045e4d4ad103ac496b3138d0edca78980b52ee2bbb6ae9bb')
        ]);

        const res = await method.handle(reqContext, 'eth_getBlockTransactionCountByNumber', ['0x1d1551'], reqId);
    ...
    rpcUnitTest('returns transaction if permission-api returns true', async ({ reqContext, rpc, authorizer }) => {
        const userAddress = '0x70997970c51812dc3a010c7d01b50e0d17dc79c8';
        authorizer.setUserAddresses([userAddress]);
    ...
    rpcUnitTest(
        ...
        async ({ reqContext, rpc, authorizer }) => {
            ...
            rpc.registerSend(`${reqId}_getBlockByNumber`, createBlockWithTxDetails([tx1, tx2, tx3]));
            rpc.registerSend(`${reqId}_getBlockReceipts`, [
    ...
    rpcUnitTest('returns null when block does not exist', async ({ reqContext, rpc, authorizer }) => {
        ...
        rpc.registerSend(`${reqId}_getBlockByNumber`, null);
    ...
    describe('when block is pending', () => {
        rpcUnitTest('returns 1 when receipt from address matches user', async ({ reqContext, rpc, authorizer }) => {
            ...
            rpc.registerSend(`${reqId}_getBlockByNumber`, createBlockWithTxDetails([tx]));

            const res = await method.handle(reqContext, 'eth_getBlockTransactionCountByNumber', ['pending'], reqId);
            expect(res).toMatchObject({
                id: reqId,
                result: '0x1'
            });
        ...
        rpcUnitTest('does not include other transactions', async ({ reqContext, rpc, authorizer }) => {
            const userAddress = '0x70997970c51812dc3a010c7d01b50e0d17dc79c8';
            const someTxHash = '0x4294364950d5b9c9045e4d4ad103ac496b3138d0edca78980b52ee2bbb6ae9bb';
            ...
            rpc.registerSend(
                `${reqId}_getBlockByNumber`,
                createBlockWithTxDetails([tx, createTransaction(pad('0x01'))])
            );
