...
export async function erc20SupplyCmd(opts: Opts): Promise<void> {
    ...
    try {
        ...
        const disclosure = await wf
            .step('Gathering proofs', 'Proofs fetched ✅', () =>
                prividiumRpc.tokenSupplyDisclosure(getAddress(opts.tokenAddr), parseBlockNumber(opts.blockNumber))
            )
            .catch((e) => {
...
export const addErc20SupplyCmd: VerifyDefCommand = (cli) =>
    cli.command(
        'erc20-total-supply <tokenAddr>',
        'Verify an ERC-20 total supply using proofs from L1',
        (yargs) =>
            yargs
